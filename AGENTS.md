...
- `app/build.gradle.kts` 当前配置：`applicationId = "com.example.campusmate"`、`minSdk = 35`、`targetSdk = 36`、`compileSdk = 36`、`compileSdkMinor = 1`（即 36.1）、`versionName = "1.0"`。
- `CampusMateDbHelper.DATABASE_NAME = "campus_mate.db"`，`DATABASE_VERSION = 5`。
- Manifest 当前声明 `POST_NOTIFICATIONS`、`FOREGROUND_SERVICE`、`FOREGROUND_SERVICE_DATA_SYNC`、`RECEIVE_BOOT_COMPLETED`、`SCHEDULE_EXACT_ALARM`、`CAMERA`、`NFC`、`INTERNET`；相机和 NFC 硬件均为 `required=false`；`allowBackup=false`。
- 已使用 Jsoup 做课表 HTML 解析，ZXing 做二维码生成和扫码，AndroidX Security Crypto 做 LLM API Key 加密存储。
- 已存在模块：Dashboard、课程管理、任务管理、任务提醒、课表导入、WebView 课表导入基础页、专注计时、翻转检测、前台服务、学习记录、热力图统计、设置页、学习计划、任务图片附件、学习名片、二维码、学习伙伴、NFC、天气、通知弱化/勿扰增强、LLM API 设置与 Client 基础设施。
- 待实现或待增强模块：项目展示页/技术点展示页、JSON 导出/备份、完整演示数据覆盖、真实教务系统 WebView 适配增强、学习计划按课程/考试生成和提醒、LLM 真实业务接入。
- 当前不做登录、云同步、后端服务、头像/照片资料、聊天、动态、关注、点赞、评论或社交广场。
- LLM 当前状态：设置页、预设、加密 Key 存储、连接测试、OpenAI-Compatible/Gemini Client、`LlmScheduleParseService` 和 `LlmPlanGenerateService` 已存在；尚未接入真实课表导入按钮或 `StudyPlanGenerator` 主流程。
...
边界：任务保存走 `TaskRepository`；提醒调度走 `AlarmReminderScheduler`；到点通知只在 `ReminderReceiver` 中处理当前任务状态后发出。修改后验证保存、编辑、完成/取消完成、删除、提醒调度、开机恢复和通知权限降级。
...
边界：当前主流程是本地规则生成器；按课程/考试生成是占位；LLM 计划生成服务未接入主流程。修改后验证今日/本周生成、重复生成提示、手动添加、完成状态、删除和详情页。
...
边界：不申请定位权限，不自动获取用户位置。默认 Mock 天气保证演示稳定；远程天气失败时保留缓存或 Mock 降级。修改后验证城市设置、Mock 开关、刷新、缓存新鲜度和无网降级。
...
- 后续课表解析目标策略是 `LLM_FIRST_FALLBACK_LOCAL`，但业务接入仍必须进入导入预览。
- 后续计划生成接入 LLM 时，不能绕过用户确认，不能直接写数据库。
