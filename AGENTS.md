...
边界：解析逻辑放在 `domain/import_`；UI 只负责输入、预览、确认和错误提示。导入必须进入 `ImportPreviewActivity`，不能静默写入。LLM 课表解析走 `LlmScheduleParseService`，输出必须由 `LlmCourseDraftValidator` 转为 `CourseDraft` 并校验课程名、星期、节次、周次和单双周，异常字段丢弃或进入 warnings，不能直接信任模型输出。WebView 不保存账号密码/Cookie，不绕过验证码；默认 BJTU 入口为 `https://mis.bjtu.edu.cn/`，用户登录后自行进入课表页；进入、点击打开和退出 `WebViewImportActivity` 时都要尽力清理 Cookie、WebStorage、缓存、历史、表单、SSL 偏好和 HTTP Auth 数据，且清理不能阻塞 UI。修改后验证 sample、粘贴 HTML、AI/本地回退、预览冲突、确认写入、导入日志、WebView 真实页面提取和再次进入需重新登录。
...
边界：二维码使用 `UserProfileRepository.buildPublicProfileJson()`；邮箱/手机号只有公开开关打开才进入 JSON；扫码接收后必须预览确认。NFC 名片交换入口、Manifest 声明和相关源码已移除，不要重新引入。修改后验证二维码生成、相机权限、扫码预览、重复提示、伙伴保存/删除。
...
新增或修改 Activity、Service、BroadcastReceiver、ContentProvider、权限、feature、前台服务、通知、相机、文件访问能力时，必须同步检查 `app/src/main/AndroidManifest.xml`。前台服务还要核对 `foregroundServiceType`、通知渠道、通知权限和停止行为。
