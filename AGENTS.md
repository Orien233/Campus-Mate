...
- `README.md` 说明当前版本为 `V1.1-stage-11`，主闭环已跑通，NFC 与天气已完成基础实现，图片附件、学习计划、JSON 导出/备份等仍属于后续计划。
...
已存在模块包括：首页、课程管理、任务管理、任务提醒、课表导入、专注计时、翻转检测、学习记录、热力图统计、设置页、学习名片、二维码、学习伙伴、NFC 和天气。

尚未完整实现或仍是计划方向的模块包括：图片附件、学习计划、NotificationListenerService、项目展示页、JSON 数据导出/备份、完整 WebView 课表导入页面。
...
- 当前数据库名为 `campus_mate.db`，数据库版本为 3。
