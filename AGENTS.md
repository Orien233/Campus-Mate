...
- `README.md` 说明当前版本为 `V1.1-stage-9`，主闭环已跑通，NFC、天气、图片附件、学习计划、JSON 导出等仍属于后续计划。
- 当前工作目录未检测到 `.git` 元数据时，Agent 不得声称已创建分支、提交或推送。
- 根目录存在 `local.properties`，但 `.gitignore` 已排除；Agent 不得提交或要求提交该文件。
- 当前版本默认不启用 Android 系统备份；不要通过系统备份导出本地数据库、设置、学习名片、学习伙伴或天气缓存。
...
已存在模块包括：首页、课程管理、任务管理、任务提醒、课表导入、专注计时、翻转检测、学习记录、热力图统计、设置页、学习名片、二维码和学习伙伴。

尚未完整实现或仍是计划方向的模块包括：NFC、天气、图片附件、学习计划、NotificationListenerService、项目展示页、JSON 数据导出/备份、完整 WebView 课表导入页面。
...
- 当前可用路径是内置 sample HTML 和粘贴 HTML；`WebViewScheduleExtractor` 已存在，但完整 WebView 导入页面尚未接入。
...
- 当前 NFC 尚未实现，Manifest 也未声明 NFC 权限。
- 实现 NFC 前必须补充 `android.permission.NFC` 和必要的 intent-filter 或前台调度逻辑。
